...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('CI'), 'has-success': fields.CI && fields.CI.valid }">
    <label for="CI" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.visit.columns.CI') }}</label>
        ...
        <input onkeypress = "return pulsar(event)" type="text"  @change="findData" v-model="form.CI"  @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('CI'), 'form-control-success': fields.CI && fields.CI.valid}" id="CI" name="CI" placeholder="{{ trans('admin.visit.columns.CI') }}">
